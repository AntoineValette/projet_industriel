...
def import_myreport_sql_general_full():
    ...
    if os.path.isfile(filename):
        ...
        log("extract myreport_sql_general_full")
        with open(filename, 'r', encoding='utf-8-sig') as f:
            ...
            for row in reader:
                ...
                """, tuple(row.values()))
        log("extract myreport_sql_general_full [ok]")
        conn.commit()
        cur.close()

        log("transform myreport_sql_general_full ...")
        log("load myreport_sql_general_full ...")
